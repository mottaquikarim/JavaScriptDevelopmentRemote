(function() { // protect the lemmings!
	...
	const onCardEntered = event => {
		const cardClicked = event.target;
		cardClicked.style.backgroundColor = 'green'
	}
	const onCardLeft = event => {
		const cardClicked = event.target;
		cardClicked.style.backgroundColor = 'red'
	}
	for (i = 0; i < cards.length; i++) {
		cards[i].addEventListener('mouseenter', onCardEntered);
		cards[i].addEventListener('mouseleave', onCardLeft);
	}
